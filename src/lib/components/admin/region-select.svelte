<script lang="ts">
  ...
  $: filtered = searchValue
    ? generations.filter((type) =>
        type?.name.toLocaleLowerCase()?.includes(searchValue?.toLocaleLowerCase()),
      )
    : generations;
...
<div class="relative group w-full" use:clickOutside on:outclick={() => (isOpen = false)}>
  <button
    ...
    id="dropdown-button"
    class="inline-flex gap-2 justify-between w-full px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-blue-500"
  >
    <div class="flex gap-2">
      <span class="mr-2"
        >{selected && selected?.id !== 0
          ? 'Gen ' + selected?.id + ' - ' + selected.name
          : 'Region'}</span
      >
    </div>
    <svg

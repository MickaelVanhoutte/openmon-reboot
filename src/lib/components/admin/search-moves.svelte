...
<div class="relative group" use:clickOutside on:outclick={() => isOpen = false}>
    <button on:click={toggleDropdown} id="dropdown-button" class="inline-flex justify-between w-full px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-blue-500">
    ...
    <div id="dropdown-menu" class="{isOpen ? 'absolute z-10 right-0 mt-2 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 p-1 space-y-1 dropdown-sizes' : 'hidden absolute right-0 mt-2 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 p-1 space-y-1'}">
      ...
      <!-- Dropdown content goes here -->
        ...
        <div on:click={() => select(entry)} class="flex justify-between items-center gap-2 gap-x-0 w-full pl-2 pr-6 border-b-blue-200">
          <div class="flex gap-2 px-4 py-2 text-gray-700 hover:bg-gray-100 active:bg-blue-100 cursor-pointer rounded-md">
            <img class="size-6 flex-none rounded-full bg-gray-50" src={"images/types/"+entry.type?.toLocaleLowerCase()+"-small.png"} alt="">

<script lang="ts">
    ...
    let isOpen = false;
    let searchValue = '';
    $:filtered = (searchValue && searchValue?.length > 2 ? types.filter((type) => type?.toLocaleLowerCase()?.includes(searchValue?.toLocaleLowerCase())) : types).sort((a, b) => a.localeCompare(b));
...
<div class="relative group w-full" use:clickOutside on:outclick={() => isOpen = false}>
    <button on:click={toggleDropdown} id="dropdown-button" class="inline-flex gap-2 justify-between w-full px-4 py-1 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-blue-500">
        <div class="flex gap-2">
            <img class="size-6 flex-none rounded-full bg-gray-50" src={"images/types/"+selected?.toLocaleLowerCase()+"-small.png"} alt="">
    ...
    <div id="dropdown-menu" class="{isOpen ? 'absolute z-10 right-0 mt-2 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 p-1 space-y-1 dropdown-sizes' : 'hidden absolute right-0 mt-2 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 p-1 space-y-1'}">
      ...
      <!-- Dropdown content goes here -->
        ...
        <div role="button" on:click={() => select(type)} class="flex justify-between items-center gap-2 gap-x-0 w-full pl-2 pr-6 border-b-blue-200">

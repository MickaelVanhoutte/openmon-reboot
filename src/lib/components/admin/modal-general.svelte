...
<div class="w-full mt-1">
  ...
  <div class="flex gap-x-2 mt-2 items-end">
    ...
    {#if pokeForm.type.length < 2}
      <button
        on:click={() => (pokeForm.type = [...pokeForm.type, 'normal'])}
        class="rounded-md bg-indigo-600 px-3 py-2 text-[0.8125rem]/5 font-semibold text-white hover:bg-indigo-500 h-fit"
        >Add Type</button
      >
    {/if}
  </div>
  ...
  <div class="flex-1 mt-10">
    ...
    <input
      ...
      min="5"
      max="255"
      bind:value={pokeForm.base.attack}
      class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
    />
    ...
    <input
      ...
      min="5"
      max="255"
      bind:value={pokeForm.base.defense}
      class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
    />
    ...
    <input
      ...
      min="5"
      max="255"
      bind:value={pokeForm.base.spAttack}
      class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
    />
    ...
    <input
      ...
      min="5"
      max="255"
      bind:value={pokeForm.base.spDefense}
      class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
    />
    ...
    <input
      ...
      min="5"
      max="255"
      bind:value={pokeForm.base.speed}
      class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
    />
  </div>
  <div>
    <p class="text-lg underline decoration-blue-600">
      Total: {total}
      {ogTotal !== total ? '(old: ' + ogTotal + ')' : ''}
    </p>
  </div>
</div>

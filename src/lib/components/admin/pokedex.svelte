...
<div class="flex h-16 gap-8 items-center justify-end">
  <button
    on:click={() => (display = display === 'lines' ? 'grid' : 'lines')}
    class="hidden md:inline-flex text-blue-700 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-1.5 text-center items-center me-2"
  >
    {#if display === 'lines'}
      <svg
...
<ul
  role="list"
  class="divide-y divide-gray-100 {display === 'grid' ? 'grid grid-cols-3 gap-8' : ''}"
>
          ...
          <p class="mt-1 truncate text-s/5 text-gray-500 flex gap-6">
            ...
            <span class="flex gap-2">
              {#each pokemon.profile.ability as ability}
                <span class="text-xs/5 text-gray-500" class:text-indigo-600={ability[1] === 'true'}
                  >{ability[0]}</span
                >
              {/each}
            </span>

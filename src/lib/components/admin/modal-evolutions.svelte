<script lang="ts">
  ...
  function update() {
    ...
    if (current.evolution.next?.length > 1) {
      multipleNext = current.evolution.next
        .map((next) => {
      ...
      while (current && current.evolution.next) {
        ...
        let found = originalPokedex.find(
          (entry) => entry.id === parseInt(current.evolution.next[0]),
        );
        if (found) {
          current = found;
          next = [...next, found];
...
<div class="flex flex-col items-center gap-5">
  ...
  {#if multipleNext?.length > 0}
    <svg
      class="block mt-0 ml-auto mr-auto"
      xmlns="http://www.w3.org/2000/svg"
      xml:space="preserve"
      width="32"
      height="32"
    ...
    <div class="w-full flex flex-wrap justify-center gap-1">
      {#each multipleNext as nex, index}
        <div class="flex flex-col items-center gap-5 mt-10 min-w-1/4 lg:min-w-1/5">
          ...
          <input
            type="text"
            class="w-full text-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-1.5"
            bind:value={pokeForm.evolution.next[index][1]}
          />
        </div>
      {/each}
    </div>
    ...
    {#each next as nex, index}
      <div class="min-w-1/2 flex flex-col items-center gap-5">
        {#if index === 0}
          <input
            type="text"
            class="w-32 text-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-1.5"
            bind:value={pokeForm.evolution.next[0][1]}
          />
          <svg
            xmlns="http://www.w3.org/2000/svg"
            xml:space="preserve"

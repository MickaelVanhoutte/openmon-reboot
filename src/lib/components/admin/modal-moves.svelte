<script lang="ts">
  ...
  $: movesByLevel = pokeForm.moves
    .filter((move) => move.method === '1')
...
<div class="flex h-16 items-center gap-5 gap-x-1">
  ...
  {#if selected}
    <button
      on:click={() => addMove()}
      class="rounded-md bg-indigo-600 w-1/2 px-3 py-2 text-[0.8125rem]/5 font-semibold text-white hover:bg-indigo-500 min-w-30"
      >Add {selected.name}</button
    >
  {/if}
</div>
      ...
      <div class="flex justify-between items-center md:gap-2 gap-x-0 w-full max-w-full">
        <div class="flex min-w-0 gap-x-4 max-w-2/3">
          <img
            ...
            src={'images/types/' + move.type?.toLocaleLowerCase() + '-small.png'}
            alt=""
          />
        ...
        <div class="flex flex-col items-end gap-2 justify-end">
          <div class="flex items-end gap-2 justify-end">
            ...
            <input
              type="number"
              class="w-16 text-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-1.5"
              bind:value={move.level}
            />
          </div>
          <button
            on:click={() => remove(move.id)}
            class="rounded-md bg-red-400 px-3 py-2 text-[0.8125rem]/5 font-semibold text-white hover:bg-red-500"
            >Remove</button
          >
        </div>
      </div>
